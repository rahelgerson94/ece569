...
float getRMSE(mf_node *R, half *P, half* Q, int numElems){ //int numRRows, int numRColumns){
  ...
  for(int i = 0; i < numElems; ++i){
      ...
      for(int k = 0; k < K; ++k)
        sum += __half2float(P[R[i].u*K + k]) * __half2float(Q[R[i].v*K + k]);
...
/*
  ...
  */
  ...
  rmse = sqrt(rmse) / numElems;
  return rmse;
}

__global__ void theirKernel(
                            mf_node *R,
                            long long numElems,
                            half *p,
                            half *q,
...
*/
    int glblIdx = blockIdx.x * blockDim.x + threadIdx.x;

    for(int update_ite = 0; update_ite < update_count_this_block; update_ite++){

    int index = glblIdx + (blockDim.x * gridDim.x) * update_ite;
    if(index < numElems) {
    //persistant thread
        ...
        */
        ...
        int rat_per_block = blockDim.x / 32;
            ...
            for(int i = 0; i < update_vector_size; i++) // 32 in Egypt paper
            {
                ...
                int indexForThisWarp = startId + local_wid + i * rat_per_block;
                int temp = randIndices[indexForThisWarp];

                float r = __ldg( &R[temp].r); //get the address of the rate field, read it from the cache
                int u = __ldg(&R[temp].u);
                int v = __ldg(&R[temp].v);
        ...
        // update_count_this_block is st in egypt paper
            ...
            */
            ...
            for(int i = 0; i < update_vector_size; i++) // 32 in Egypt paper
            {
                ...
                float ruv = (r - tmp_product) * (r+4.9999)/5; //get error , ignoring zeros
...
mf_node* readInputAsSparseMF(float* input, int numRows, int numCols, int* sparseLength){
  mf_node* inputMF = (mf_node *) malloc(numRows * numCols * sizeof(mf_node));
  *sparseLength = 0;

  for(int u = 0; u < numRows; u++){
    for(int v = 0; v < numCols; v++){
      if(input[u*numCols + v] != 0) {
        inputMF[u*numCols+v].u = u; // row index
        inputMF[u*numCols+v].v = v; // column index
        inputMF[u*numCols+v].r = input[u*numCols+v]; // rating value
        (*sparseLength)++;
...
int main(int argc, char **argv) {
  ...
  if(SPARSE) 
    hostR = readInputAsSparseMF(temp,numRRows,numRColumns,&numElems);
  else {
  ...
  cudaMalloc((void **) &deviceR, numElems*sizeof(mf_node));

  wbTime_stop(GPU, "Allocating GPU memory.");

  wbTime_start(GPU, "Copying input memory to the GPU.");
  //@@ Copy memory to the GPU here
  cudaMemcpy(deviceR,hostR,numElems*sizeof(mf_node),cudaMemcpyHostToDevice);
  ...
  int numBlocks = (numElems-1)/no_r_b + 1;

  int st = 1;
  if(numBlocks > 32){
    numBlocks = 32;
    st = (numElems-1)/(no_r_b*numBlocks) + 1;
  }

  dim3 mygrid(numBlocks);
  dim3 myblock(no_r_b);
  ...
  sgd_k128_kernel_hogwild_warp32_lrate<<<mygrid,myblock>>>(
    ...
    rmse = getRMSE(hostR, hostP, hostQ, numElems); // in future can make this GPU kernel but for now we do serial
    printf("\n");
